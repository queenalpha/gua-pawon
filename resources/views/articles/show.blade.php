...
@section('artikel-konten')
    ...
    <div class="max-w-6xl mx-auto flex flex-col md:flex-row">
        <div class="w-full md:w-24 flex md:flex-col items-center justify-start">
            ...
            <div class="flex md:flex-col items-center md:mt-5 text-sm text-gray-500 space-x-1">
                <span class="md:mb-1 ms-2">{{ \Carbon\Carbon::parse($article->created_at)->translatedFormat('F') }}</span>
                <span
                    class="text-base lg:text-4xl lg:font-bold md:mb-1 md:mt-1  ">{{ \Carbon\Carbon::parse($article->created_at)->format('d') }}</span>
        ...
        ->replace('<img', '<img style="max-width: 50%; height: auto; margin: 2rem auto; display: block;"') //ubah lebar menjadi 50%
    ...
    <div class="max-w-6xl mx-auto px-4 mb-10 mt-12">
        ...
        @else
            <p class="text-center text-gray-500">{{__('articles/show.belum-ada-artikel-terkait')}}
